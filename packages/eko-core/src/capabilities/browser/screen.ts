...
/**
 * Browser screen capability abstract class
 * 
 * Extends BrowserBaseCapability with screenshot-based interaction tools.
 * Provides tools for typing, clicking, scrolling, etc. in browser context.
 */
export abstract class BrowserScreenCapability extends BrowserBaseCapability {
  constructor() {
    super();
    // Merge base tools with screen-specific tools
    this._tools = [...this._tools, ...this.buildScreenTools()];
  }

  /**
   * Abstract methods to be implemented by concrete browser screen capability implementations
   */
  ...
  /**
   * Build screen-specific tools
   */
  private buildScreenTools(): Tool[] {
    return [
      {
        name: "typing",
   ...
   */
  getGuide(): string {
    return `You are a browser operation agent, use a mouse and keyboard to interact with a browser.
